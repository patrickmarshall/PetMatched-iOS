...
public final class DAOHistoryListBaseClass: NSCoding {
  ...
  public var errorMsg: DAOHistoryListErrorMsg?
  public var response: [DAOHistoryListResponse]?
  ...
  public required init(json: JSON) {
    ...
    errorMsg = DAOHistoryListErrorMsg(json: json[SerializationKeys.errorMsg])
    if let items = json[SerializationKeys.response].array { response = items.map { DAOHistoryListResponse(json: $0) } }
  ...
  required public init(coder aDecoder: NSCoder) {
    ...
    self.errorMsg = aDecoder.decodeObject(forKey: SerializationKeys.errorMsg) as? DAOHistoryListErrorMsg
    self.response = aDecoder.decodeObject(forKey: SerializationKeys.response) as? [DAOHistoryListResponse]
